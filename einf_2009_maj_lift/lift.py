...
with open("blokkol.txt", "w", encoding="UTF-8") as fileKi:
    for igeny in igenyek:
        if igeny.csapat == csapat:
            ...
            fileKi.write(f"Indulási emelet: {igeny.honnan}\n")
            fileKi.write(f"Cél emelet: {igeny.hova}\n")
            fileKi.write(f"Feladatkód: {feladatkod}\n")
            fileKi.write(f"Befejezés ideje: {igeny.ora}:{igeny.perc}:{igeny.mperc}\n")
            fileKi.write(f"Sikeresség: {sikeresseg}\n")
            fileKi.write(f"-----\n")
