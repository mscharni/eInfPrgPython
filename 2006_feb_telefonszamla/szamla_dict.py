...
with open("hivasok.txt", "r") as fileBe:
    while True:
        line1 = fileBe.readline().strip()   # időadat
        if line1 != "":
            line2 = fileBe.readline().strip()   # telefonszám
            hivas = {
                'ido'   : line1,                         # hívás tól-ig eredeti formában
                'kora'  : int(line1.split(" ")[0]),      # hívás kezdetének órája
                'perc'  : idotartam(line1),              # hívás időtartama percben
                'tszam' : line2,                         # telefonszám
                'tkor'  : line2[0:2],                    # körzetszám
                'ttip'  : 0 if line2[0:2] in mobilkorzet else 1   # 0 = Mobilszám | 1 = egyéb szám
            }
            hivasok.append(hivas)
        else:
            break
print("Adatok beolvasva a 'hivasok.txt' állományból.")
with open("percek_dict.txt", "w") as fileKi:
