...
# egy objektumban helyezzük el az összetartozó adatokat, amit egy listában gyűjtünk
# objektum deklarálása
class Adas:
    def __init__(self,datum, cim, epizod, perc, latta):
        self.datum = datum
        self.cim = cim
        self.epizod = epizod
        self.perc = perc
...
for i in range(0, len(elemek), 5):
    ...
    # rövidebb verzió
    """
    adat = Adas(elemek[i].strip(), elemek[i+1].strip(),elemek[i+2].strip(),int(elemek[i+3].strip()), int(elemek[i+4].strip()))
    """
    # hosszabb, de jobban olvasható verzió
    adat = Adas(
...
print("\n8. feladat")
with open('summa_obj_in_list.txt', 'w') as file:
    elozo = adatok[0]
    ...
    file.write(f"{elozo.cim} {per} {ep}")
print(f"Adatok kiírva a 'summa_obj_in_list.txt' állományba!")